import { Hono } from "hono";
import { TipoPagoController } from "../controllers/TipoPagoController";
import { requirePlatformAuthority } from "./global-catalog-authority";

export const tipoPagoRoutes = new Hono();
const controller = new TipoPagoController();

tipoPagoRoutes.get("/", (c) => controller.list(c));
tipoPagoRoutes.get("/:id", (c) => controller.getById(c));
tipoPagoRoutes.post("/", requirePlatformAuthority, (c) => controller.create(c));
tipoPagoRoutes.put("/:id", requirePlatformAuthority, (c) => controller.update(c));
tipoPagoRoutes.delete("/:id", requirePlatformAuthority, (c) => controller.delete(c));
